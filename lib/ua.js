...
async function navigate(ua, url, undici_params) {
    ...
    params.headers ??= {}
    if (ua.alwaysHeaders) Object.assign(params.headers, ua.alwaysHeaders)
  ...
  and body ${params.body}`)
    ...
    } else if (resp.headers['www-authenticate'] === 'Negotiate' && undici_params?.method === 'POST') {
        throw { error: `Unexpected header « WWW-Authenticate: Negotiate » in POST response` }
